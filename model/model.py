...
#from resnet.torch import resnet101
from utils.helpers import *
 


class ResBlock(nn.Module):
    """ Residual block. Composed of two ReLu operations and two 3x3 convolutions. 
    Used in the decoder. Both the refinement module with the skip connections and the read value from the STMemoryRead. 
    """
...
class Encoder_M(nn.Module):
    """ Memory Encoder Class. Gets a frame and it's annotation and 
    outputs Key and Value for that particular frame.
    """
...
class Encoder_Q(nn.Module):
    """ Query Encoder Class. Gets a frame with no current annottaion and
    outputs Key and Value for that particular frame. Key used to comute similarities with 
    memory values and Value combined with memory values to perform Decoding of the annotation.
    """
    def __init__(self, backbone):
...
class Decoder(nn.Module):
    ...
    def forward(self, r4, r3, r2):
        # r4 -> Read value from memory space time read
        m4 = self.ResMM(self.convFM(r4))
        # r3 -> Skipped connections from Query encoder Resnet3
        # r2 -> Skipped Connections from Query encoder Resner2
        ...
        if self.backbone == 'resnest101':
            ...
            # Pases though last 3x3 convolution
            p2 = self.pred2(F.relu(m2))
        
        # Upscales final image by a size of 4. 
